...
- The merged runtime contract now loads `prayers.json` before `book.json` and exposes prayers as `Î∂ÄÎ°ù:Í∏∞ÎèÑÎ¨∏`.
...
- time: 2026-05-29 23:58:00 +09:00
  task: chapter 1 learning comic update
  summary: adjusted VerseView comic mode so chapter 1 now shows exactly one image per verse by verse-number lookup in «–Ω¿∏∏»≠/1
  status: done
